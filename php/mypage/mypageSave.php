        ...
        if($result){
            ...
            if($memberInfo['youPass'] == $youPass && $memberInfo['memberID'] == $memberID){
                ...
                if($fileExtension == "jpg" || $fileExtension == "jpeg" || $fileExtension == "png" || $fileExtension == "gif"){
                    $sql = "UPDATE myMember SET youEmail = '{$youEmail}', youIntro = '{$youIntro}', youNickName = '{$youNickName}', youSite = '{$youSite}', youName = '{$youName}', youBirth = '{$youBirth}', youPhone = '{$youPhone}', youPass = '{$youPass}', youPhoto = '{$myPageName}' WHERE memberID = '{$memberID}'";
                    $connect -> query($sql);
                    $result = $connect -> query($sql);
                    $result = move_uploaded_file($myPageTmp, $myPageDir.$myPageName);
     
                } else {
                    $sql = "UPDATE myMember SET youEmail = '{$youEmail}', youName = '{$youName}', youBirth = '{$youBirth}', youPhone = '{$youPhone}', youNickName = '{$youNickName}', youIntro = '{$youIntro}', youSite = '{$youSite}' WHERE memberID = '{$memberID}'";
                    $connect -> query($sql);
                    $result = $connect -> query($sql);
                    $result = move_uploaded_file($myPageTmp, $myPageDir.$myPageName);
                }
